  ...
  async fetch(req) {
    ...
    try {
      ...
      if (votes) {
        ...
        // Vain välilyönnit koodataan; kauttaviiva säilyy polkuerottimena.
        const path = tunnus.split('/').map(encodeURIComponent).join('/');
        const url = `${EDK_BASE}/taysistunnot/asian-aanestykset/${path}`;
        const r = await fetch(url, {
          headers: { Accept: 'application/json', 'User-Agent': 'ACI-policy-proxy/1.0' }
        });
        const text = await r.text();
        if (!r.ok) throw new Error(`Äänestykset ${tunnus}: ${r.status} ${text.slice(0,180)}`);
        let data;
        ...
        return Response.json({
          ...
          status: rows.length === 0
            ? 'ei aanestyksia — asiasta ei ole aanestetty (EI virhe)'
            : 'aanestyksia loytyi',
          data: rows
        }, { headers: CORS });
      }
