    ...
    try {
      ...
      if (votes) {
        ...
        const classify = (v) => {
          ...
          if (/luottamus/.test(t))
            return { kind: 'luottamusaanestys', usable: false, party_line: null,
                     why: 'aanestyksen kohde on luottamus, ei sisalto — '
                        + 'uptakea ei voi lukea tuloksesta' };

          // RYHMÄKURI on MERKINTÄ, EI ESTE.
          //
          // KORJATTU 2026-09-07. Ensimmäinen versio merkitsi täydellisen
          // hallitus–oppositio-jaon epävarmaksi (usable=null). Se oli
          // KÄSITTEELLINEN VIRHE: se sekoitti SELITYKSEN ja MITTAUKSEN
          // ESTEEN.
          //
          // HE 101/2024:ssä on 13 äänestystä, kaikki vastalauseita
          // ("mietintö / Pinja Perholehto"). Hallitus äänesti 99–0
          // jokaisessa. Vastaehdotukset hävisivät — ja se ON uptake = 0
          // havaintona: kanta äänestettiin ja hylättiin. Ryhmäkuri
          // selittää MIKSI se hävisi, muttei tee tuloksesta
          // mittaamatonta.
          //
          // Sivuhuomio joka paljasti virheen: alkuperäinen sääntö vaati
          // MOLEMPIEN puolien puhtautta eikä lauennut kertaakaan, koska
          // oppositio hajosi 15–55. Kynnyksen löysääminen olisi
          // korjannut väärän asian.
          const ho = v.hallitusoppositioJakaumat || [];
          const hal = ho.find(x => /hallitus/i.test((x.nimi || {}).fi || ''));
          const partyLine = hal ? (hal.ei === 0 || hal.jaa === 0) : null;

          return {
            kind: vaihe.includes('toinen') ? 'sisaltoaanestys-2k' : 'sisaltoaanestys',
            usable: true,
            party_line: partyLine,
            why: 'sisalto aanestettiin — havio on uptake = 0 havaintona'
               + (partyLine === true
                   ? '. HUOM: hallitus aanesti yksimielisesti, eli tulos '
                     + 'seuraa hallituspohjaa. Se selittaa tuloksen muttei '
                     + 'estä mittausta.'
                   : ''),
          };
        };
        ...
        const marked = rows.map(v => {
          const c = classify(v);
          return { ...v, _vote_kind: c.kind, _uptake_usable: c.usable,
                   _party_line: c.party_line, _uptake_note: c.why };
        });
        ...
        return Response.json({
          ...
          // Ryhmäkuri raportoidaan erikseen: se on selittävä tieto, ei
          // suodatin. Kutsuja voi halutessaan painottaa sillä.
          party_line_votes: marked.filter(v => v._party_line === true).length,
          data: marked
        }, { headers: CORS });
      }
